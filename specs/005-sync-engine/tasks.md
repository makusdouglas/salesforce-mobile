...
- [X] T035 Re-run the Supabase schema verification queries from [contracts/supabase-schema.md §5](./contracts/supabase-schema.md#5-verification-checklist-for-the-admin) against the dev project to confirm all seven tables have both new columns and the trigger. Attach the output to the PR description.
- [X] T036 Manually walk through [quickstart.md §(a) Verification](./quickstart.md#verification) on both a phone simulator (390×844) and a tablet simulator (820×1180). For each viewport, confirm each of the four indicator states appears correctly and the pull-to-refresh gesture works. This is the UX5 tablet compliance check for the component's placement in its host (the indicator itself is layout-neutral; the check validates the host renders it correctly on both form factors).
