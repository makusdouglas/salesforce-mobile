...
**Intent selection is a single conditional.** `sendOrder` inspects `order.clientEmail` through a single helper `isLikelyEmail` (exported from `clientSlug.ts`) and either opens `expo-mail-composer` with `recipients: [email], subject, body, attachments: [pdfPath]` (FR-003) or falls back to `expo-sharing.shareAsync(pdfPath, { mimeType: 'application/pdf', UTI: 'com.adobe.pdf' })` when the email is missing or fails the shape check (FR-004 + edge case). The hint component reads the same `isLikelyEmail` output so button label and hint copy can never disagree (FR-005/006). The mail path consumes `MailComposerResult.SENT` precisely (cancel ≠ sent); the share path treats any non-throwing resolve as `sent` — see research.md R11 for the Android rationale.

**Single choke point for opening stored PDFs.** Both the OrderSummary "Ver PDF salvo" action (shown when status = `sent`) and the OrderSent screen's "Ver PDF salvo" CTA call the same helper `openStoredPdf(orderId)` exported from `src/features/orders/send/openStoredPdf.ts`. That helper owns the FR-017 missing-file branch: check `FileSystem.getInfoAsync`; if missing, call `orderSendService.regeneratePdfForSentOrder(orderId)` which reuses the persisted `order_number`. This keeps regeneration consistent across every entry point.
