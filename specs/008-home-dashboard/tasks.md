...
- [ ] T014 [P] Create TWO files for the catalog summary:
...
- [ ] T016 [P] Create TWO files for the clients summary:
...
- [ ] T017 [P] Write `src/features/home/tests/clientsSummary.test.ts` testing ONLY the pure `deriveClientsSummary` helper: null → `{count: 0}`; empty array → `{count: 0}`; populated array → count matches length.
- [ ] T018 [P] Create `src/features/home/hooks/useDraftsSummary.ts` — placeholder body `return { count: 0 };` with TODO(009-orders) comment. Signature: `() => { count: number }`. Because the body is a constant (no other hook calls), it is callable directly from Node as a plain function in tests.
- [ ] T019 [P] Write `src/features/home/tests/useDraftsSummary.test.ts`: `expect(useDraftsSummary()).toEqual({ count: 0 })`. Locks the contract that 009-orders must preserve when swapping the body.
- [ ] T020 [P] Create `src/features/home/hooks/useLastSentOrder.ts` — placeholder body `return null;` with TODO(009-orders) comment. Signature: `() => RecentActivityDTO | null` (the DTO type lives in `src/features/home/types.ts` — add it here if not yet present). Same "constant body, callable from Node" property as T018.
- [ ] T021 [P] Write `src/features/home/tests/useLastSentOrder.test.ts`: `expect(useLastSentOrder()).toBeNull()`.
...
- [ ] T021a [P] Create TWO files for the modal:
