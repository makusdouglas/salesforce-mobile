...
- [ ] T018 Edit `src/app/providers/AppProviders.tsx` — wrap the existing provider tree with both `<LockProvider>` (inside `<SessionProvider>`) and `<PrivacySnapshotView />` (at the outermost level, so it also covers the `NotAuthenticated` / login screen state). Final shape:
  ```tsx
  <PrivacySnapshotView>
    <SessionProvider>
      <LockProvider>
